...
fig.add_trace(go.Scatter(x=np.fft.fftfreq(len(waveform_1), d=(time_s[1] - time_s[0])), y=np.abs(fourier_transform_1), mode='markers', name='Fourier Transform 1'), row=1, col=2)
fig.add_trace(go.Scatter(x=time_s, y=waveform_2, mode='lines', name='Waveform 2'), row=2, col=1)
fig.add_trace(go.Scatter(x=np.fft.fftfreq(len(waveform_2), d=(time_s[1] - time_s[0])), y=np.abs(fourier_transform_2), mode='markers', name='Fourier Transform 2'), row=2, col=2)
fig.add_trace(go.Scatter(x=time_s, y=waveform_3, mode='lines', name='Combined Waveform'), row=3, col=1)
fig.add_trace(go.Scatter(x=np.fft.fftfreq(len(waveform_3), d=(time_s[1] - time_s[0])), y=np.abs(fourier_transform_3), mode='markers', name='Fourier Transform of Combined Waveform'), row=3, col=2)
fig.add_trace(go.Scatter(x=time_s, y=waveform_4, mode='lines', name='Clipped Combined Waveform'), row=4, col=1)
fig.add_trace(go.Scatter(x=np.fft.fftfreq(len(waveform_4), d=(time_s[1] - time_s[0])), y=np.abs(fourier_transform_4), mode='markers', name='Fourier Transform of Clipped Combined Waveform'), row=4, col=2)
